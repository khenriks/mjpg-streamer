...
Return Value: -
******************************************************************************/
void switch_resolution(input *in, uint32_t width, uint32_t height, int rotation, uint32_t buffercount)
{
    context *pctx = (context*)in->context;
    ...
    while (!pglobal->stop) {
        ...
        if (in->snapshot) {
            if (width != pctx->videoIn->snapshot_width || height != pctx->videoIn->snapshot_height) {
                ...
                switch_resolution(in, pctx->videoIn->snapshot_width, pctx->videoIn->snapshot_height, rotation, 1);
                is_switch = 1;
            }
            in->snapshot = 0;
        }
        ...
        if (is_switch) {
            switch_resolution(in, width, height, rotation, buffercount);
            is_switch = 0;
        }
    }
